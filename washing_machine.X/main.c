...
void main(void) {
    ...
    clcd_print(" Press SW5 To  ", LINE1(0));
...
{
    clcd_putch(0xFF, LINE1(i));  // Full black block
}
clcd_print("  Powering ON   ", LINE2(0));
clcd_print(" Washing Machine ", LINE3(0));
for(unsigned char i = 0; i < 16; i++)
{
    clcd_putch(0xFF, LINE4(i));  // Full black block
}

for (unsigned char pos = 0; pos < 16; pos++) {
    clcd_putch(0xFF, LINE1(pos));   // Black box on Line 1
    clcd_putch(0xFF, LINE4(pos));   // Black box on Line 4
    __delay_ms(100);                // Small delay for animation
    clcd_putch(' ', LINE1(pos));    // Clear Line 1 position
    clcd_putch(' ', LINE4(pos));    // Clear Line 4 position
}
__delay_ms(1000);
}
